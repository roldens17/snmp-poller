...
func (s *Service) evaluateAlerts(ctx context.Context, orgID int64, deviceID int64, pollTime time.Time, metrics []snmpclient.InterfaceMetric, prev map[int]store.InterfaceState, prevCounters map[int]store.InterfaceCounters) {
	for _, metric := range metrics {
		...
		if ifDown && ok {
			...
			if durationDown >= s.cfg.Alerting.InterfaceDownAfter.Duration && prevState.OperStatus != "" {
				s.raiseAlert(ctx, orgID, deviceID, metric.Index, "interface_down", "warning", fmt.Sprintf("%s (%d) down for %s", metric.Name, metric.Index, durationDown.Round(time.Second)))
				continue
			}
		}
		if !ifDown {
			s.clearAlert(ctx, orgID, deviceID, metric.Index, "interface_down")
		}
		...
		if ratio >= s.cfg.Alerting.ErrorRateThreshold && errorDelta > 0 {
			s.raiseAlert(ctx, orgID, deviceID, metric.Index, "error_rate", "warning", fmt.Sprintf("%s error rate %.2f%%", metric.Name, ratio*100))
		} else {
			s.clearAlert(ctx, orgID, deviceID, metric.Index, "error_rate")
		}
		...
		if metric.Speed > 0 && timeDelta > 0 && octetDelta > 0 {
			...
			if bps >= threshold {
				s.raiseAlert(ctx, orgID, deviceID, metric.Index, "bandwidth", "info", fmt.Sprintf("%s %.2f%% utilization", metric.Name, (bps/float64(metric.Speed))*100))
			} else {
				s.clearAlert(ctx, orgID, deviceID, metric.Index, "bandwidth")
			}
		}
	}
}

func (s *Service) raiseAlert(ctx context.Context, orgID int64, deviceID int64, ifIndex int, category, severity, message string) {
	idx := ifIndex
	alert := store.Alert{
		OrgID:    orgID,
		DeviceID: deviceID,
...
func (s *Service) clearAlert(ctx context.Context, orgID int64, deviceID int64, ifIndex int, category string) {
	idx := ifIndex
	if err := s.store.ResolveAlert(ctx, orgID, deviceID, &idx, category); err != nil {
